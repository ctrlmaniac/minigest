...
import aziendaExists from "~/features/azienda/exists";
import update from "~/features/azienda/update";
...
const Form: React.FC<Props> = ({ dettagli }) => {
  ...
  const { exists, putting, putError, response } = useAppSelector(
    (state) => state.azienda
  );
  ...
  const handleSubmit = () => {
    ...
    if (!putError) {
      setTimeout(() => {
        navigate(`/aziende/dettagli/${dettagli.id!}`);
      }, 3000);
    }
  };
  ...
  return (
    <>
      ...
      <SaveFab onClick={handleSubmit} disabled={isDisabled} loading={putting} />
    </>
  );
};
