...
import update from "~/features/f24/update";
...
const Form: React.FC<Props> = ({ dettagli }) => {
  ...
  const { putting, putError, response } = useAppSelector((state) => state.f24);

  /**
   * VALORI COMUNI
   */
  const [values, setValues] = React.useState({
    dataScadenza: dettagli.dataScadenza,
    dataPagamento: dettagli.dataPagamento || "",
  });
  ...
  const [erario, setErario] = React.useState({
    codiceUfficio: dettagli.sezioneErario
      ? dettagli.sezioneErario.codiceUfficio
      : "",
    codiceAtto: dettagli.sezioneErario ? dettagli.sezioneErario.codiceAtto : "",
  });
  ...
  const [erarioReparti, setErarioReparti] = React.useState<any>(
    dettagli.sezioneErario ? dettagli.sezioneErario.reparti : []
  );
  ...
  const [locali, setLocali] = React.useState({
    detrazione: dettagli.sezioneTributiLocali
      ? dettagli.sezioneTributiLocali.detrazione
      : "",
    identificativoOperazione: dettagli.sezioneTributiLocali
      ? dettagli.sezioneTributiLocali.identificativoOperazione
      : "",
  });
  ...
  const [localiReparti, setLocaliReparti] = React.useState<any>(
    dettagli.sezioneTributiLocali ? dettagli.sezioneTributiLocali.reparti : []
  );
  ...
  const handleSubmit = () => {
    ...
    if (erarioReparti.length > 0) {
      ...
      if (!isEmpty(dettagli.sezioneErario)) {
        payload.sezioneErario = {
          ...payload.sezioneErario,
          id: dettagli.sezioneErario.id,
        };
      }
    }
    ...
    if (localiReparti.length > 0) {
      ...
      if (!isEmpty(dettagli.sezioneTributiLocali)) {
        payload.sezioneTributiLocali = {
          ...payload.sezioneTributiLocali,
          id: dettagli.sezioneTributiLocali.id,
        };
      }
    }
    ...
    if (!putError) {
      ...
      navigate("/fisco/f24/dettagli/" + dettagli.id!);
    }
  };
  ...
  return (
    <>
      <Box mb={3}>
        <Typography variant="h3">Modifica F24</Typography>
      </Box>
